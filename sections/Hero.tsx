...
export default function Hero() {
  ...
  return (
    <section className="relative h-[220px] overflow-hidden sm:h-[180px] lg:h-[200px]">
      <div className="absolute inset-0 overflow-hidden">
        <Image
          src={locale === "ar" ? "/images/hero-road-v4.jpg" : "/images/hero-road-eng.jpg"}
          alt="Masar Makers"
          fill
          priority
          sizes="100vw"
        ...
        <div className="absolute inset-0 z-10">
          <div
            className={`mx-auto flex h-full max-w-7xl items-center ${
              locale === "ar"
                ? "pl-[136px] pr-3 sm:pl-6 sm:pr-[190px] lg:pl-8 lg:pr-[160px]"
                : "pl-3 pr-[136px] sm:pl-[190px] sm:pr-5 lg:pl-[100px] lg:pr-8"
            }`}
          >
            <div
              dir={locale === "ar" ? "rtl" : "ltr"}
              className={`relative w-full min-w-0 max-w-[760px] text-white ${
                locale === "ar" ? "ml-auto text-right" : "mr-auto text-left"
              }`}
            >
              <h1 className="text-[14px] font-black leading-[1.35] text-white sm:whitespace-nowrap sm:text-[14px] sm:leading-tight lg:text-[18px] xl:text-[28px]">
                {locale === "ar" ? (
                  <>
                    لا تتعلم كورس فقط ...{" "}
              ...
              <p className="mt-1.5 max-w-[610px] text-[9px] font-bold leading-[14px] text-white/90 sm:mt-1 sm:text-[9px] sm:leading-5 lg:text-[14px]">
                {text.description}
              </p>

              <div className="mt-3 flex flex-wrap gap-1.5 sm:mt-4 sm:gap-2">
                <AuthLink href="/dashboard">
                  <button className="flex items-center gap-1 rounded-xl bg-[#F7B548] px-3 py-1.5 text-[9px] font-bold text-[#07152E] transition duration-300 hover:scale-105 hover:shadow-[0_0_30px_rgba(247,181,72,.45)] sm:rounded-2xl sm:px-2 sm:text-[11px] lg:text-[12px]">
                    {text.start}
                    <ArrowLeft size={13} className="sm:h-[15px] sm:w-[15px]" />
                  </button>
                </AuthLink>

                <AuthLink href="/career-path/road-design">
                  <button className="flex items-center gap-1.5 rounded-xl border border-white/30 bg-white/10 px-3 py-1.5 text-[9px] font-bold text-white backdrop-blur transition duration-300 hover:bg-white hover:text-[#07152E] sm:rounded-2xl sm:px-2 sm:text-[11px] lg:text-[12px]">
                    {text.explore}
                    <Compass size={13} className="sm:h-[15px] sm:w-[15px]" />
                  </button>
                </AuthLink>
              </div>
            </div>
