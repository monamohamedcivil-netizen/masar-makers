...
export async function readImportExcel(
  ...
  return rows.map((row) => ({
    ...
    studentEmail:
      String(row["Student Email"] ?? "")
        .trim()
        .toLowerCase(),

    jobTitle:
      String(row["Job Title"] ?? "").trim(),
...
journeyType:
  String(
    ...
    detailedSurveyCompleted:
      normalizeBoolean(
        row["Detailed Survey Completed"],
      ),

    showReviewHome:
      normalizeBoolean(
        row["Show Review Home"],
      ),

    showReviewCourse:
      normalizeBoolean(
        row["Show Review Course"],
      ),
    ...
    showProjectHome:
      normalizeBoolean(
        row["Show Project Home"],
      ),

    showProjectCourse:
      normalizeBoolean(
        row["Show Project Course"],
      ),
