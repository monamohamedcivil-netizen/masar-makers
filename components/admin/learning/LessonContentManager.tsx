...
export default function LessonContentManager({
  ...
  useEffect(() => {
    const bunnyLessons = lessons.filter(
      (lesson) =>
        lesson.video_provider === "bunny" &&
        lesson.video_asset_id,
    );

    if (bunnyLessons.length === 0) return;

    let cancelled = false;
    let syncing = false;

    async function syncAllBunnyLessons() {
      if (cancelled || syncing) return;
      syncing = true;

      try {
        const results = await Promise.all(
          bunnyLessons.map(async (lesson) => ({
            lessonId: lesson.id,
            result: await syncBunnyVideoMetadata(lesson.id),
          })),
        );
        ...
        setLessons((current) =>
          current.map((lesson) => {
            ...
            if (!synced || !synced.result.success) return lesson;

            const seconds = Number(
              synced.result.data.durationSeconds || 0,
            );

            return {
              ...lesson,
              video_status: synced.result.data.status,
        ...
        console.error("AUTO SYNC LESSON VIDEOS ERROR", error);
      } finally {
        syncing = false;
      }
    }

    void syncAllBunnyLessons();
      ...
      <div className="max-h-[94vh] w-full max-w-4xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
        ...
        <div className="grid gap-5 p-6 md:grid-cols-2">
          ...
          <div className="rounded-xl border border-slate-200 p-3">
            ...
            <p className="mt-1 text-sm font-black text-[#C88712]">
              Bunny Stream
            </p>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 p-3">
            ...
            <p className="mt-1 text-sm font-black text-[#C88712]">
              {Number(editing.duration_minutes || 0) > 0
                ? `${editing.duration_minutes} دقيقة`
                : "تُحسب تلقائيًا من Bunny"}
            </p>
          </div>
          ...
          <div className="md:col-span-2">
            {editing.id ? (
              <BunnyVideoUploader
                ...
                currentVideoId={
                  editing.video_asset_id ||
                  null
                }
                currentStatus={
                  editing.video_status ||
                  null
                }
                onChanged={(data) =>
  setEditing({
