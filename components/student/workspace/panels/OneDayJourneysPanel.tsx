...
type Props = { groups: StudentOneDayJourneyGroup[] };

export default function OneDayJourneysPanel({ groups }: Props) {
  if (!groups.length) {
    return (
      <div className="flex min-h-[400px] flex-col items-center justify-center text-center">
        ...
        <h3 className="mt-4 text-xl font-black text-[#07152E]">لا توجد رحلات يوم واحد بعد</h3>
        <p className="mt-2 max-w-sm text-sm font-semibold text-slate-500">ستظهر هنا الورش التي تم اعتماد اشتراكك بها.</p>
      </div>
    );
  }
      ...
      {groups.map((group) => (
        <section key={group.id} className="overflow-hidden rounded-[22px] border border-[#DCE2EA] bg-white shadow-[0_12px_32px_rgba(7,21,46,0.07)]">
          <header className="flex items-center justify-between border-b border-[#E5EAF0] bg-[#F7F9FC] px-5 py-3">
            ...
            <span className="rounded-full bg-[#FFF4DF] px-3 py-1 text-[10px] font-black text-[#B5790C]">{group.journeys.length} محاضرات</span>
          </header>
            ...
            {group.journeys.map((journey) => {
              ...
              return (
                <div key={journey.enrollmentId} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-5 py-3 sm:grid-cols-[180px_minmax(0,1fr)_150px]">
                  ...
                  <Link href={journey.href} className="inline-flex h-10 items-center justify-center gap-2 rounded-[12px] bg-[#07152E] px-4 text-[11px] font-black text-white transition hover:-translate-y-0.5 hover:bg-[#10284F]">
